...
parser.add_argument('--input', '-i', required=True,
                    help='input wikipedia dump file (.bz2)')
parser.add_argument('--output', '-o', default='',
                    help='output file name')
parser.add_argument('--count', '-c', type=int, default=-1,
                    help='number of wikipedia articles to output')
parser.add_argument('--category-file', '-cf', default=None,
                    help='category file of wikipedia articles')
...
with open(args.category_file, 'r') as f:
    input_categories = f.read().splitlines()
    input_categories = [c.replace(u'\u200e', '') for c in input_categories]
...
def get_categories(text):
    ...
    categories = RE_P14.findall(text)
    categories = [p.match(c).group(1) for c in categories if p.match(c)]
    return categories
...
def sentence_tokenize(content):
    """
    Tokenize a piece of wikipedia text in to sentences
    """
    tokens = []
...
class WikiCorpus(TextCorpus):
    ...
    def __init__(self, fname, processes=None, lemmatize=utils.has_pattern(), dictionary=None, filter_namespaces=('0',)):
        ...
        #     self.dictionary = Dictionary(self.get_texts_sent_split())
        # else:
    ...
    #             tokens_all = [t for s in tokens for t in s]
    #             articles_all += 1
    #             positions_all += len(tokens_all)
    #             # article redirects and short stubs are pruned here
    #             if len(tokens_all) < ARTICLE_MIN_WORDS or any(title.startswith(ignore + ':') for ignore in IGNORED_NAMESPACES):
    #                 continue
    #             articles += 1
    #             positions += len(tokens_all)
    #             if self.metadata:
    #                 yield (tokens_all, (pageid, title))
    #             else:
    #                 yield tokens_all
    #     pool.terminate()
    #
...
def process_wiki():
    wiki = WikiCorpus(args.input)
    wiki.metadata = True
    model = wiki.get_texts_sent_split()

    if args.count >= 0:
        model = itertools.islice(model, args.count)

    with open(args.output, 'w') as f:
        for tokens, (page_id, title) in model:
            f.write(title + '\n')
            i = 0
            for sent in tokens:
                if len(sent) >= 5:
                    f.write(title + '_SENT%d\n' % i)
                    f.write(' '.join([t.decode('utf-8') for t in sent]) + '\n')
                    i += 1
